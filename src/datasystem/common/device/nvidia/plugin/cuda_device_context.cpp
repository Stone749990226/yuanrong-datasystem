...
{
    // CUDA does not have a direct API for stream synchronization with timeout.
    // Implement using polling with cudaStreamQuery.
    if (timeoutMs < 0) {
        return cudaStreamSynchronize(stream);
    }
...
{
    // CUDA does not have a direct API for event synchronization with timeout.
    // Implement using polling with cudaEventQuery.
    if (timeoutMs < 0) {

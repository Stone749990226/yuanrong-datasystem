...
public:
    ...
    virtual Status MemcpyBatchD2H(const std::vector<DeviceBlobList> &devBlobList, std::vector<Buffer *> &bufferList);
    virtual Status MemcpyBatchH2D(const std::vector<DeviceBlobList> &devBlobList, std::vector<Buffer *> &bufferList);
    virtual void SetPolicyByHugeTlb(bool enableHugeTlb);
...
protected:
    ...
    uint64_t deviceMemSize = 2684354560;  // 2.5G
    uint64_t hostMemSize = 104857600;     // 100MB
private:

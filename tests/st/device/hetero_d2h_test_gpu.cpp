...
class HeteroD2HGpuTestEvcit : public HeteroD2HGpuTest {
    void SetClusterSetupOptions(ExternalClusterOptions &opts) override
    {
        opts.numWorkers = 1;
        opts.workerGflagParams =
...
class HeteroD2HThroughTcpGpuTest : public DevTestHelper {
    void SetClusterSetupOptions(ExternalClusterOptions &opts) override
    {
        auto workerNum = 2;
...
TEST_F(HeteroD2HThroughTcpGpuTest, SetGet)
{
    const int32_t setDeviceId = static_cast<int32_t>(GetDeviceIdFromEnv("DS_TEST_DEVICE_ID", 0));
    const int32_t getDeviceId = static_cast<int32_t>(GetDeviceIdFromEnv("DS_TEST_DEVICE_ID_2", setDeviceId));
...
TEST_F(HeteroD2HGpuTest, TestNoExist)
{
    const int32_t deviceId = static_cast<int32_t>(GetDeviceIdFromEnv("DS_TEST_DEVICE_ID", 0));
...
TEST_F(HeteroD2HGpuTest, TestAllExist)
{
    const int32_t deviceId = static_cast<int32_t>(GetDeviceIdFromEnv("DS_TEST_DEVICE_ID", 0));
    std::vector<uint64_t> keyNums{ 450, 500 };
    std::vector<std::vector<double>> costs(keyNums.size());
        ...
        if (numOfObjs == 450) {
            ...
            for (auto &blobList : devSetBlobList) {
                for (auto &blob : blobList.blobs) {
                    ...
                    DS_ASSERT_OK(
                        CudaDeviceManager::Instance()->MemCopyH2D(blob.pointer, blob.size, data.data(), blob.size));
                }
            }
...
TEST_F(HeteroD2HGpuTest, TestPartExist)
{
    const int32_t deviceId = static_cast<int32_t>(GetDeviceIdFromEnv("DS_TEST_DEVICE_ID", 0));
    std::vector<uint64_t> keyNums{ 450, 500 };
    std::shared_ptr<HeteroClient> client;
...
TEST_F(HeteroD2HGpuTestEvcit, SpillTest)
{
    const int32_t deviceId = static_cast<int32_t>(GetDeviceIdFromEnv("DS_TEST_DEVICE_ID", 0));
...
TEST_F(HeteroD2HGpuTest, TestMSetD2HMsgWithInvalidDeviceId)
{
    const int32_t deviceId = static_cast<int32_t>(GetDeviceIdFromEnv("DS_TEST_DEVICE_ID", 0));
    InitCuda(deviceId);
    std::shared_ptr<HeteroClient> client;
